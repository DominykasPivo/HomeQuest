...
class Property(models.Model):
    ...
    seller = models.ForeignKey(Seller, on_delete=models.CASCADE, related_name='properties')  # One-to-Many relationship
    property_id = models.AutoField(primary_key=True)
    location = models.CharField(max_length=255)
    map_location = models.CharField(max_length=255, blank=True, null=True)  # Optional
    price = models.DecimalField(max_digits=10, decimal_places=2)
    size = models.DecimalField(max_digits=10, decimal_places=2)  # Size in square meters
    ...
    images = models.ManyToManyField('Image', blank=True, related_name='properties')  # Many-to-Many for images
...
class Comment(models.Model):
    property = models.ForeignKey(Property, on_delete=models.CASCADE, related_name='comments')  # One-to-Many relationship
    user = models.ForeignKey(User, on_delete=models.CASCADE)  # User who made the comment
    content = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Comment by {self.user.email} on Property {self.property.property_id}"
